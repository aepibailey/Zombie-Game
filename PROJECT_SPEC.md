...
- **Debug:** `J` toggles an on-screen list of every zombie within footstep range with its distance, to verify the ~20m threshold by walking toward one. (rebound off F3 — no action in this project may be bound to a function key)
...
- **`K`** toggles translucent head/body hitbox volumes; **`J`** toggles the audible-zombie distance overlay. (rebound off F4/F3 — no action in this project may be bound to a function key)

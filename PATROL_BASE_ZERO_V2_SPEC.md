...
- `K` hitbox debug volumes are type-driven, so they draw the correct
  silhouette per variant rather than always the walker's. (rebound off F4 —
  no action in this project may be bound to a function key)
